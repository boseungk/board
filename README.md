...
* 스프링 부트를 통해 웹 애플리케이션을 만들면서 내부가 어떻게 돌아가는지 알 수 없어서 답답함을 느꼈습니다. 그래서 스프링의 기반 기술인 Http와 Servlet에 대해서 공부하기 시작했습니다. 덕분에 스프링의 기반 기술인 Servlet이 어떤 식으로 Http 메시지를 처리하는지 이해하게 되었습니다. 예를 들어서 Redirection의 경우 Servlet을 통해 Http 응답 메시지에 3XX 응답의 결과를 넣습니다. 그럼 브라우저가 Http 응답 메시지을 읽고 리다이렉션이 이뤄지게 됩니다. 이를 통해 스프링 내부에서 어떤 일이 일어나는지 더 깊게 알게 되었습니다.

#### 4-2. 객체지향의 장점에 대한 이해
* 스프링 부트로 웹 애플리케이션을 만들면서 '왜 번거롭게 인터페이스를 이용해서 클래스를 구현하는 걸까?'라는 의문을 가지게 되었습니다. 이에 대한 답을 찾기 위해서 추가로 객체지향에 대해 공부를 하게 되었습니다. 좋은 객체지향 설계는 구체화에 의존하지 않고 추상화에만 의존합니다. 덕분에 부품을 바꿔 끼우듯 클래스의 교체가 쉬워집니다. 따라서 인터페이스는 변경하지 않고 구현된 클래스만 수정하기 때문에 추가적인 유지 보수 비용이 줄어듭니다. 이런 지식을 통해 스프링이 객체지향적으로 매우 잘 설계된 기술이라는 것을 알게 되었습니다.

#### 4-3. Web MVC 구조와 REST API와 AJAX에 대한 이해
* 이 프로젝트를 진행하기 전에는 서버에서 어떤 일들이 일어나는지 알지 못했고, 서버가 어떻게 웹과 모바일을 동시에 처리할 수 있는지 궁금했었습니다. 프로젝트를 진행 하면서 스프링의 Web MVC 구조를 직접 구현해보았고, 기본적인 Web MVC 구조를 이해할 수 있었습니다. 예를 들어 FrontController에서 공통적인 사용자의 요청을 처리한 후 해당 URL에 해당하는 Controller에게 전달됩니다. 그럼 Service를 통해 서비스 로직을 처리하고 DB로부터 데이터를 받아옵니다. 그 후 Model에 담아 사용자에게 View로 출력합니다. 이 프로젝트에서는 Thymeleaf를 통해서 서버에서 view를 그려주는 방식으로 구현했지만, 간단하게 React를 학습하면서 Ajax 기술의 발달로 클라이언트와 서버가 JSON 데이터만을 주고받기 때문에 웹과 모바일을 전부 처리할 수 있다는 것을 알 수 있었습니다. 이 과정에서 URL(URI)의 의미가 '행위나 작업'에서 '자원 식별'로 변화했고, HTTP 메서드를 통해서 자원인 데이터의 CRUD가 이뤄진다는 점도 알게 되었습니다.

#### 4-4. 데이터베이스와 JPA 등 추가적인 공부의 필요성
* 게시판에 댓글 기능을 추가하고 싶었지만 1:N과 같은 데이터베이스 설계에 대한 이해가 필요했습니다. 3학년 1학기에 배우는 데이터베이스 시스템 과목을 수강하고 추가적인 JPA에 대한 학습을 진행할 예정입니다. 이외에도 프로젝트를 진행하면서 TDD(테스트 주도 개발)에 대해서 알게 되었습니다. 다음에는 이 방법론을 적용시키면서 추가적인 프로젝트를 진행 해보려고 합니다. 
