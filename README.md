...
* 이 프로젝트를 진행하기 전에는 서버에서 어떤 일이 일어나는지, 서버가 어떻게 웹과 모바일을 동시에 처리할 수 있는지 궁금했었습니다. 하지만 이 프로젝트로 스프링의 Web MVC 구조를 직접 구현해보면서 기본적인 Web MVC 구조를 이해할 수 있게 되었습니다. 예를 들어 FrontController에서 공통적인 사용자의 요청을 처리한 후 해당 URL에 해당하는 Controller에게 전달됩니다. 그럼 Service를 통해 서비스 로직을 처리하고 DB로부터 데이터를 받아옵니다. 그 후 Model에 담아 사용자에게 View로 출력합니다. 이 프로젝트에서는 Thymeleaf를 통해서 서버에서 view를 그려주는 방식으로 구현했지만, 따로 React를 간단하게 학습하면서 Ajax 기술의 발달로 클라이언트와 서버가 JSON 데이터만을 주고받기 때문에 웹과 모바일 전부를 처리할 수 있다는 것을 알 수 있었습니다. 이 과정에서 URL(URI)의 의미가 '행위나 작업'에서 '자원 식별'로 변화했고, HTTP 메서드를 통해서 자원인 데이터의 CRUD가 이뤄진다는 점도 알게 되었습니다.
